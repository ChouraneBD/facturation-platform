...
const prepareLine = async (line) => {
    const article = line.article_id
        ? await Article.findByPk(line.article_id, { include: [Category] })
        : null;
...
const calculateTotals = (preparedLines, methodeCalcul, remiseGlobalePct) => {
    ...
    const totalBaseHt = preparedLines.reduce((sum, line) => sum + line.baseHt, 0);
    const totalLineRemise = preparedLines.reduce((sum, line) => sum + roundMoney(line.baseHt * (line.remise_pct / 100)), 0);
    ...
    return {
        ...
        remise_globale_pct: Number(remiseGlobalePct || 0),
...
const listFactures = async (req, res) => {
    try {
        ...
        const filteredFactures = filterFactures(factures, req.query);

        return res.status(200).json(filteredFactures);
    } catch (error) {
...
const updateGlobalDiscount = async (req, res) => {
    try {
        ...
        const preparedLines = facture.lignes_facture.map((line) => ({
            ...
            lineTva: roundMoney(
                Number(line.quantite) * Number(line.prix_unitaire_applique) * (1 - Number(line.remise_pct || 0) / 100) * Number(line.tva_pct) / 100
            ),
            total_ligne: roundMoney(
                Number(line.quantite) * Number(line.prix_unitaire_applique) * (1 - Number(line.remise_pct || 0) / 100) * (1 + Number(line.tva_pct) / 100)
            ),
        ...
        return res.status(200).json({ message: 'Remise globale mise à jour avec succès.', facture });
    } catch (error) {
